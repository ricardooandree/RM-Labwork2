...
public class DiameterShClient implements DiameterProvider, DiameterListener
{
      ...
      if(request.getCommandCode() == DiameterShCodes.PUSH_NOTIFICATION_REQUEST)
      {
        ...
        if(!user2update.getUser().isEmpty()) // this user exists in the charging database
        {
          ...
          if(userState.equals("1")) 
          {
              ...
              // Send SIP Message
              DiameterOpenIMSSipServlet.sendSIPMessage( userPublicIdentity, user2update.getNotification()); // Message
          }
  ...
  public void receivedSuccessMessage( Request request, Answer answer )
  {
    ...
    // RESERVAR 40 CREDITOS - 20 TAXACAO FIXA + 20 2 MINS CHAMADA 
    // START TIMER - TIMER É DE 2 MINS
    // TIMER EXPIRED && SESSAO ATIVA = RENOVA TIMER
    // CASO O CREDITO FIQUE NEGATIVO => TRANSMITE MENSAGEM
    // SESSAO TERMINADA => SIP BYE => STOP TIMER E RESTITUIR O SALDO RESERVADO NAO CONSUMIDO + ENVIAR MENSAGEM C/ SALDO DOS USERS
